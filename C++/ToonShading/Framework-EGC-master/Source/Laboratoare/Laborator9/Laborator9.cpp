...
void Laborator9::Init()
{
	...
	{
		mapTextures["random"] = CreateRandomTexture(24, 24);
	}
...
void Laborator9::Update(float deltaTimeSeconds)
{
...

	{
		...
		RenderSimpleMesh(1, meshes["box"], shaders["ShaderLab9"], modelMatrix, mapTextures["crate"]);
	}

	{
		...
		RenderSimpleMesh(2, meshes["box"], shaders["ShaderLab9"], modelMatrix, mapTextures["random"]);
	}
...
void Laborator9::RenderSimpleMesh(int type, Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
	...
	int loc_type = glGetUniformLocation(shader->program, "type");
	glUniform1i(loc_type, type);
