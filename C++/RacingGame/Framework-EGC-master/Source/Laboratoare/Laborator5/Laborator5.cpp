...
void Laborator5::Update(float deltaTimeSeconds)
{
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::scale(modelMatrix, glm::vec3(1 / 20.0f, 1 / 20.0f, 1 / 20.0f));
		modelMatrix = glm::translate(modelMatrix, glm::vec3(-2, 0.5f, 0));
...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_O)) {
			...
			// TODO :  face trecerea in proiectie ortografica
			projectionMatrix = glm::ortho(-5.0f, +5.0f, -5.0f, +5.0f, 0.1f, 200.0f);
		}
...
void Laborator5::OnMouseMove(int mouseX, int mouseY, int deltaX, int deltaY)
{
	...
	if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->GetSpecialKeyState() == 0) {
			...
			camera->RotateFirstPerson_OX(-deltaY * sensivityOY * 10);
			camera->RotateFirstPerson_OY(-deltaX * sensivityOX * 10);
		}
