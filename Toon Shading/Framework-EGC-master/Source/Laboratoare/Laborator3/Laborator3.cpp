...
void Laborator3::Update(float deltaTimeSeconds)
{
	...
	if (fflag == 0)
	{
		a = deltaTimeSeconds * 2;
		b = deltaTimeSeconds * 2;
		fflag = 1;
	}
	
	if (scalex > 5 && fflag == 1)
	{
		a = (-1)* deltaTimeSeconds * 2;
		b = (-1)* deltaTimeSeconds * 2;
		fflag = 2;
	}
	if (scalex < 1 && fflag == 2)
	{
	...
	scalex += a;
	scaley += b;
	modelMatrix *= Transform2D::Translate(150, 300)* Transform2D::Scale(scalex, scaley)*Transform2D::Translate(-50, -50);
	...
	if (flag == 0)
	{
		patrat2 = deltaTimeSeconds * 50;
		flag = 1;
	}
	if (txg > 200 && flag == 1)
	{
		patrat2 = (-1)*deltaTimeSeconds * 100;
		flag = 2;
	}
	if (txg < 10 && flag == 2)
	{
		patrat2 = deltaTimeSeconds * 100;
		flag = 1;
	}
		
	txg += patrat2;
	modelMatrix *= Transform2D::Translate(txg, 0);


	//TODO create animations by multiplying current transform matrix with matrices from Transform 2D
	RenderMesh2D(meshes["square2"], shaders["VertexColor"], modelMatrix);

	trads += deltaTimeSeconds * 5;
	modelMatrix = glm::mat3(1);
	modelMatrix *= Transform2D::Translate(800, 400) * Transform2D::Rotate(trads) * Transform2D::Translate(-50, -50);
