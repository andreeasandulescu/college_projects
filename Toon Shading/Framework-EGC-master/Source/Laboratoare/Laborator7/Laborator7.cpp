...
void Laborator7::Update(float deltaTimeSeconds)
{
	...
	// Render ground
	{
		...
		RenderSimpleMesh(meshes["plane"], shaders["ShaderLab7"], modelMatrix,glm::vec3(0,1,1));
	}
...
void Laborator7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	int colorobj = glGetUniformLocation(shaderID, "object_color");
	glUniform3fv(colorobj, 1, glm::value_ptr(color));

	int light = glGetUniformLocation(shaderID, "light_position");
	glUniform3fv(light, 1, glm::value_ptr(this->lightPosition));

	// TODO: Set eye position (camera position) uniform
	int camera = glGetUniformLocation(shaderID, "eye_position");
	glm::vec3 eyePosition = GetSceneCamera()->transform->GetWorldPosition();
	glUniform3fv(camera, 1, glm::value_ptr(eyePosition));

	// TODO: Set material property uniforms (shininess, kd, ks, object color) 
	int matKd = glGetUniformLocation(shaderID, "material_kd");
	int matKs = glGetUniformLocation(shaderID, "material_ks");
	int matShininess = glGetUniformLocation(shaderID, "material_shininess");

	glUniform1f(matKd, this->materialKd);
	glUniform1f(matKs, this->materialKs);
	glUniform1i(matShininess, this->materialShininess);
