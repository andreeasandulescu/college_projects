...
//	modelMatrix *= Transform3D::Translate(-2.5f, 0.5f,-1.5f);
	...
	modelMatrix = glm::mat4(1);
	modelMatrix *= Transform3D::Translate(translateX, translateY, translateZ);
	modelMatrix *= Transform3D::RotateOY(angularStepOYaux);
	modelMatrix *= Transform3D::Translate(2.0f, 0.0f, 0.0f);
	RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);

	modelMatrix = glm::mat4(1);
	modelMatrix *= Transform3D::Translate(translateXaux, translateYaux, 0.0f);
	modelMatrix *= Transform3D::RotateOZ(angularStepOZaux);
	RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
}
...
{
	if (window->KeyHold(GLFW_KEY_W) && (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT) == 0) )
		translateZ += deltaTime * 2;
	if (window->KeyHold(GLFW_KEY_S) && (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT) == 0))
		translateZ -= deltaTime * 2;
	if (window->KeyHold(GLFW_KEY_A) && (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT) == 0))
		translateX -= deltaTime * 2;
	if (window->KeyHold(GLFW_KEY_D) && (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT) == 0))
		translateX += deltaTime * 2;
	if (window->KeyHold(GLFW_KEY_R) && (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT) == 0))
		translateY -= deltaTime * 2;
	if (window->KeyHold(GLFW_KEY_F) && (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT) == 0))
		translateY += deltaTime * 2;
	...
	{
		angularStepOYaux += deltaTime;
	}
	if (window->KeyHold(GLFW_KEY_N))
	{
		translateXaux += 2 * deltaTime;
		translateYaux = abs(sin (translateXaux));
		angularStepOZaux -= deltaTime;
	}
